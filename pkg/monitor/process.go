...
func NewProcessMonitorFromEmbed() (*ProcessMonitor, error) {
	candidates := []string{"/usr/lib/edr/bpf/process_events.o", "bpf/process_events.o", "process_events.o"}
	exe, _ := os.Executable()
	if exe != "" {
		exeDir := filepath.Dir(exe)
		candidates = append(candidates, filepath.Join(exeDir, "bpf/process_events.o"), filepath.Join(exeDir, "process_events.o"))
	}
